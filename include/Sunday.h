...
// 언두 작업 종류
#define DO_INSERT    1 // 문자 입력, 붙여넣기 등
#define DO_DELETE    2 // 문자 삭제, 잘라내기 등

#define LINE_HEIGHT    18 // AA 한 줄 높이(dot)

#define RULER_AREA    13 // 편집 창 눈금 영역 높이

#define LINENUM_WID    37 // 편집 창 행 번호 영역 너비
#define LINENUM_COLOUR        0xFF8000 // 편집 창 행 번호 영역 색상
...
// 여러 파일을 다룰 때는 이 구조를 한 단계 더 감싸서 사용한다.
...
BOOLEAN        FrameEditHandleMessage( LPMSG );
VOID        FrameEditDestroy( VOID );
HRESULT        FrameNameLoad( UINT, LPTSTR, UINT_PTR ); // 지정한 말풍선 이름을 반환
...
BOOLEAN        ViewDrawCaret( INT, INT , BOOLEAN ); // 실제로는 그리기보다 위치 변경에 가깝다.
...
UINT        DocDelayPageLoad( FILES_ITR , INT ); // 지연 페이지 로드
