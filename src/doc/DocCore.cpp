...
// 지정한 페이지 전체를 미리보기용 텍스트로 확보한다. free는 호출 측에서
// 처리한다. 現在のユニコード数値参照の具合に合わせて文字列をチェインジ
HRESULT UnicodeRadixExchange(VOID)
{
    ...
    for (dP = 0; iPage > dP; dP++) //    全頁
    {
        ...
        for (dL = 0; iLine > dL; dL++, itLine++) //    全行
        {
            ...
            {
                if (itLine->vcLine.at(dM).mzStyle & CT_NOSJISREF)
                {
                    ...
                    StringCchCopyA(itLine->vcLine.at(dM).acNoSjisText, 10, acNoSjisText);
                    //    TODO:    バイト数再計算が必要
                }
            }
        }
